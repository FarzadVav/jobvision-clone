...
const IranAnimation = () => {
  ...
  const activeRandomCircle = () => {
    ...
    const nextCircleElem = document.querySelector<SVGCircleElement>(`#dott-${number}`)
  ...
  return (
    <svg viewBox="0 0 800 710" fill="none" xmlns="http://www.w3.org/2000/svg">
      <g id="Group 5">
      ...
      {svgPath.x !== 0 && svgPath.y !== 0 ? (
        ...
        >
          ...
          >
            <div
              className={`bg-white border-t border-l border-solid border-[#00000025] w-3 h-3 rotate-45 absolute -top-[0.39rem] z-0`}
            ></div>
            <span className={`text-[60%] z-10`}>یک رزومه برای شرکت دیجیکالا در تهران ارسال شد</span>
          </div>
        </foreignObject>
      ) : null}
    </svg>
